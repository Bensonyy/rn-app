...
import { getUserUnconfirmOrderList } from '../../../../api';
import {
  Button,
  Card,
  RootContext,
...
const WaitOrderTake = () => {
  ...
  const fetchData = async () => {
    ...
    setRefreshing(true);
    const { result, data } = await getUserUnconfirmOrderList.fetch(userId);
    if (result === 'success') {
      setListData(data);
    }
    setRefreshing(false);
...
const Item = ({ item }) => {
  ...
  return (
    <TouchableWithoutFeedback
      onPress={() => {
        navigation.navigate('WaitOrderTakeDetail', {
