...
const EditAddress = () => {
  ...
  useEffect(() => {
    ...
    setProvinceName(params.provinceName);
    setCityName(params.cityName);
    setCountyName(params.countyName);
    setLongitude(params.longitude);
    setLatitude(params.latitude);
    setAreaName(`${params.provinceName}${params.cityName}${params.countyName}`);
    return () => {};
  }, []);
  ...
  useEffect(() => {
    let params = route?.params;
    setValue('linkName', params.linkName);
    setValue('linkTel', params.linkTel);
    setValue('streetAddress', params.streetAddress);
    return () => {};
  }, []);
  ...
  const getValidate = useCallback(() => {
    if (typeof areaName === 'undefined' || areaName == null || areaName == '') {
  ...
  const onSubmit = async (formData) => {
    ...
    const { result } = await editAddress.fetch(sendData);
    if (result === 'success') {
      Toast({
        message: '修改成功',
      });
